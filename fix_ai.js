...
// Remove static GRAMMAR_MISTAKES array and logic
content = content.replace(/\/\/ Comprehensive grammar mistake detector[\s\S]*?(?=export async function chatWithAITutor)/, '');
content = content.replace(/let detectedCorrection: GrammarCorrection \| undefined;\s+if \(isEnglishAttempt\) \{[\s\S]*?\}\s+(?=const geminiKey)/, '');

// Upgrade Prompt
const oldPrompt = /const prompt = `You are Coach Maya[\s\S]*?suggestions: \["suggestion 1", "suggestion 2", "suggestion 3"\]\n  }`;/
const newPrompt = `const prompt = \`You are Coach Maya, an encouraging, incredibly smart, empathetic, and professional English teacher for LinguaLearn.
  ...
  3. SMART CORRECTIONS:
     ...
     - If you correct them, explicitly explain WHY they were wrong in the "explanation" field (e.g. "We say 'on Monday', not 'in Monday', because we use 'on' for days of the week.")
