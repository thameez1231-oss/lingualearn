...
interface LessonRunnerClientProps {
  lessonId: string;
  initialUserLanguage: string;
}

export default function LessonRunnerClient({ lessonId, initialUserLanguage }: LessonRunnerClientProps) {
  const lesson = getLessonById(lessonId);

  const [userLanguage, setUserLanguage] = useState(initialUserLanguage);
  
  // Start straight at exercises if there is no vocabulary (e.g. Checkpoints)
  const initialStage = lesson?.vocabulary && lesson.vocabulary.length > 0 ? 'vocab' : 'exercises';
  const [currentStage, setCurrentStage] = useState<'vocab' | 'exercises' | 'completed' | 'failed'>(initialStage);
                ...
                <button
                  ...
                  onClick={() => {
                    if (isCorrect) {
                      ...
                      setSentenceTokens([]);
                      setAvailableTokens(
                        currentExercise.type === 'sentence_builder' && currentExercise.scrambledWords
