...
export async function sendEmail({ to, subject, html, text, type = 'VERIFICATION' }: SendEmailParams) {
  const from = process.env.SMTP_FROM || 'LinguaLearn <noreply@lingualearn.app>';
  ...
  if (previewUrl) {
    console.log(`\n========================================`);
    console.log(`📧 [LinguaLearn Email Sent]`);
    console.log(`To: ${to}`);
...
export async function sendVerificationEmail(email: string, name: string, token: string) {
  ...
  const html = `
    ...
    <html>
      <head>
        ...
        <title>Verify your LinguaLearn Email</title>
      </head>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 30px; color: #1e293b;">
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
          <div style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); padding: 32px 24px; text-align: center;">
            ...
            <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">LinguaLearn</h1>
          ...
          <div style="padding: 32px 28px;">
            ...
            <p style="font-size: 13px; color: #94a3b8; margin-top: 24px;">
              This link will expire in 24 hours. If you did not sign up for LinguaLearn, you can safely ignore this email.
            </p>
          </div>

          <div style="border-top: 1px solid #f1f5f9; padding: 20px; text-align: center; font-size: 12px; color: #94a3b8; background: #fafafa;">
            © ${new Date().getFullYear()} LinguaLearn Inc. • EdTech for Beginners
          </div>
        </div>
      </body>
    </html>
  ...
  return sendEmail({
    to: email,
    subject: 'Confirm your LinguaLearn account',
    html,
    type: 'VERIFICATION',
  });
}
...
export async function sendPasswordResetEmail(email: string, name: string, token: string) {
  ...
  const html = `
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <title>Reset your LinguaLearn Password</title>
      </head>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f8fafc; margin: 0; padding: 30px; color: #1e293b;">
        <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; overflow: hidden;">
          <div style="background: #0f172a; padding: 28px 24px; text-align: center;">
            <h1 style="color: #ffffff; margin: 0; font-size: 22px; font-weight: 700;">LinguaLearn Security</h1>
          </div>

          <div style="padding: 32px 28px;">
            <h2 style="font-size: 20px; font-weight: 700; color: #0f172a; margin-top: 0;">Password Reset Request</h2>
            <p style="font-size: 15px; line-height: 1.6; color: #475569;">
              Hi ${name}, we received a request to reset your LinguaLearn password. Click the button below to create a new password.
            </p>
